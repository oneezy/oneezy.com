...
<svelte:head>
  <title>Portfolio {SITE_SEPARATOR} {SITE_TITLE}</title>
  <meta name="description" content="The portfolio for Justin O'Neill">
  
  <meta property="og:title" content="Portfolio {SITE_SEPARATOR} {SITE_TITLE}" />
...
<main class="py-40 md:py-56" in:fly={{ y: 50, duration: 500, delay: 500 }} out:fade>
  ...
  <div class="portfolio-projects container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
    {#each portfolio as project}
        ...
        <a href="/portfolio/{project.label}" sveltekit:prefetch class="flex relative w-full h-64 rounded-xl overflow-hidden group">
          <img class="transition absolute inset-0 w-full h-full object-cover object-center" src="/portfolio/{project.label}/img.png" alt="{project.name}">
